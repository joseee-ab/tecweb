...
<?php
    ...
    if(!empty($producto)) {
        ...
        if($consulta = mysqli_query($conexion, $sql)) {
            ...
            if ($marca !== $row['marca'])        
                $cambios[] = "marca='$marca'";
            if ($modelo !== $row['modelo'])      
                $cambios[] = "modelo='$modelo'";
            if ($precio != $row['precio'])       
                $cambios[] = "precio=$precio";
            if ($detalles !== $row['detalles'])  
                $cambios[] = "detalles='$detalles'";
            if ($unidades != $row['unidades'])   
                $cambios[] = "unidades=$unidades";
            if ($imagen !== $row['imagen'])      
                $cambios[] = "imagen='$imagen'";
        }

       if(count($cambios) > 0) {
        $sql = "UPDATE productos SET " .implode(", ", $cambios) . " WHERE nombre='{$jsonOBJ->nombre}'"; 
